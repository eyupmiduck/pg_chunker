/*
    runs a test insert operation into the 2 char target table from the source table
    validates the rows to make sure they are identical
*/
CREATE OR REPLACE PROCEDURE chunk_test_pgplsql.test_insert_test_table_int_int_pk(
    i_chunk_size IN INTEGER
)
LANGUAGE plpgsql
AS $$
DECLARE
    l_table_name    VARCHAR(4000) := 'test_table_int_int_pk';
BEGIN
    ...
    CALL chunk_pgplsql.execute_dml(
        ...
        i_run_name            => 'test_insert_test_table_int_int_pk'
    );
...
BEGIN
    ...
    FOR l_chunk_size IN i_lower_chunk_size..i_upper_chunk_size LOOP
        CALL chunk_test_pgplsql.test_insert_test_table_int_int_pk(
