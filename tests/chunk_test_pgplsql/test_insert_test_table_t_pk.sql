...
AS $$
DECLARE
    l_table_name    VARCHAR(4000) := 'test_table_t_pk';
BEGIN
    ...
    CALL chunk_pgplsql.execute_dml(
        ...
        i_statement_text      => '
            INSERT INTO chunk_test_data_target.test_table_t_pk(key1 , row_value)
            SELECT t.key1 , t.row_value
            FROM <driving_table>
            WHERE  <primary_key_range>',
        i_chunk_size          => i_chunk_size,
        i_run_name            => 'test_insert_test_table_t_pk'
    );
...
BEGIN
    ...
    FOR l_chunk_size IN i_lower_chunk_size..i_upper_chunk_size LOOP
        CALL chunk_test_pgplsql.test_insert_test_table_t_pk(
