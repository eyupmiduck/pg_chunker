/*
    Get all the possible statements that will be used to execute chunks on a table with 2 col pk
*/
CREATE OR REPLACE PROCEDURE chunk_pgplsql_lib.pk2_get_chunk_statements(
    ...
    o_k1_e_k2_n_key_stmt    OUT VARCHAR,
...
BEGIN
    ...
    CALL chunk_pgplsql_lib.p_replace(
        io_source   => o_k1_e_k2_n_key_stmt,
        i_old       => chunk_pgplsql_const.primary_key_range_tag(),
        i_new       => l_k1_e_k2_n_key_key
    );
    CALL chunk_pgplsql_lib.p_replace(
    ...
    CALL chunk_pgplsql_lib.log_statements(
        i_log_level         => i_log_level,
        i_stmt_description  => 'Key 1 equal Key 2 not equal statement',
        i_stmt_1            => o_k1_e_k2_n_key_stmt
    );
    CALL chunk_pgplsql_lib.log_statements(
        i_log_level         => i_log_level,
        i_stmt_description  => 'Key 1 not equal statements',
        i_stmt_1            => o_k1_n_1_stmt,
        i_stmt_2            => o_k1_n_1_stmt,
        i_stmt_3            => o_k1_n_1_stmt
    );
    CALL chunk_pgplsql_lib.log_statements(
        i_log_level         => i_log_level,
